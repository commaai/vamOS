...
/*
 * Phase 2.B (4.9 -> 6.18 IOMMU port):
 *
 * The legacy driver used the ARM legacy IOMMU mapping API (asm/dma-iommu.h:
 * arm_iommu_create_mapping/attach/detach/release, struct dma_iommu_mapping)
 * and the downstream msm_dma_iommu_mapping helpers (msm_dma_map_sg_lazy /
 * msm_dma_unmap_sg). All of that was removed from mainline arm64 in the 5.x
 * series.
 *
 * Ported to the generic mainline IOMMU API:
 *   - Per context-bank explicit iommu_domain via iommu_paging_domain_alloc(dev),
 *     bound with iommu_attach_device()/iommu_detach_device(), freed with
 *     iommu_domain_free(). This faithfully reproduces the legacy behaviour: the
 *     driver never used the dma-iommu/DMA-API IOVA allocator for its data path
 *     - it manages its own IOVA space (SHARED via gen_pool, SCRATCH via a
 *     private bitmap, FIRMWARE/QDSS/SECHEAP at fixed DT IOVAs) and maps buffers
 *     into the domain itself with iommu_map()/iommu_map_sg(). An explicit
 *     iommu_domain is exactly that primitive, so every existing
 *     iommu_map[_sg]/iommu_unmap call site is preserved verbatim (only the
 *     cb->mapping->domain indirection collapses to cb->domain, and the new
 *     gfp argument is added).
 *   - The IO region previously mapped via the downstream lazy mapper now uses a
 *     gen_pool IOVA allocator over the IO region + iommu_map_sg(), mirroring the
 *     SHARED path. This yields stable, driver-controlled device IOVAs for
 *     camera buffers - the contract callers (cam_isp/cam_icp/...) rely on.
 *   - iommu_domain_set_attr()/DOMAIN_ATTR_* were removed in mainline; the
 *     non-fatal-fault / iova-guard attributes are dropped (they were tuning
 *     hints, not correctness requirements for bring-up).
 *
 * The SECURE (stage-2) ION/SCM path is out of scope for 2.B - it is shimmed
 * (cam_smmu_compat.h) and belongs to buckets 2.A (ion->dma-buf) and 2.E (scm).
 */
...
struct cam_context_bank_info {
	struct device *dev;
	/*
	 * 6.18 port: was `struct dma_iommu_mapping *mapping` (ARM legacy API).
	 * Now an explicit generic iommu_domain that the driver allocates,
	 * attaches and maps into itself.
	 */
	struct iommu_domain *domain;
	dma_addr_t va_start;
	size_t va_len;
	...
	/*
	 * 6.18 port: the IO region used to be mapped by the downstream lazy
	 * mapper (msm_dma_map_sg_lazy) which owned its own IOVA space. With the
	 * explicit domain we allocate IO-region IOVAs ourselves from this pool
	 * and iommu_map_sg() into the domain (mirrors shared_mem_pool).
	 */
...
static int cam_smmu_attach_device(int idx)
{
	...
	/* attach the domain to device (generic IOMMU API) */
	rc = iommu_attach_device(cb->domain, cb->dev);
	if (rc < 0) {
		CAM_ERR(CAM_SMMU, "Error: IOMMU attach failed. ret = %d",
			rc);
		rc = -ENODEV;
	}
...
/*
 * 6.18 port: IO-region IOVA allocator. The legacy driver delegated IO-region
 * IOVA management to the downstream lazy mapper (msm_dma_map_sg_lazy). With the
 * explicit generic domain we own the IOVA space ourselves, allocated from a
 * gen_pool over the DT-described IO region (see cam_smmu_setup_cb). This keeps
 * device IOVAs stable and driver-controlled - the contract callers rely on.
 */
static int cam_smmu_alloc_io_iova(int idx, size_t size, dma_addr_t *iova)
{
	unsigned long vaddr;

	if (!iommu_cb_set.cb_info[idx].io_mem_pool) {
		CAM_ERR(CAM_SMMU, "IO mem pool not initialised");
		return -ENODEV;
	}

	/* gen_pool granularity is PAGE_SIZE; round up so unmap len matches. */
	vaddr = gen_pool_alloc(iommu_cb_set.cb_info[idx].io_mem_pool,
		PAGE_ALIGN(size));
	if (!vaddr)
		return -ENOMEM;

	*iova = (dma_addr_t)vaddr;
	return 0;
}

static void cam_smmu_free_io_iova(int idx, dma_addr_t iova, size_t size)
{
	if (!iommu_cb_set.cb_info[idx].io_mem_pool)
		return;

	gen_pool_free(iommu_cb_set.cb_info[idx].io_mem_pool, iova,
		PAGE_ALIGN(size));
}
	...
	struct cam_dma_buff_info **mapping_info)
{
	...
	if (region_id == CAM_SMMU_REGION_SHARED) {
		...
		/*
		 * 6.18 port: replaces the downstream msm_dma_map_sg_lazy(). We
		 * allocate an IOVA from the IO-region gen_pool and iommu_map_sg()
		 * the buffer's scatterlist into our explicit domain, mirroring
		 * the SHARED path. dma_addr is now a driver-owned IOVA rather
		 * than sg_dma_address() (the DMA API was never wired to this
		 * private domain).
		 */
		dma_addr_t io_iova = 0;
		size_t buf_len = 0;
		...
		if (rc < 0) {
			CAM_ERR(CAM_SMMU, "IOVA alloc failed for IO memory");
			goto err_unmap_sg;
		}

		/*
		 * Map RW: camera IO buffers are read and/or written by the HW
...
err_alloc:
	if (region_id == CAM_SMMU_REGION_SHARED) {
		...
		iommu_unmap(iommu_cb_set.cb_info[idx].domain,
			*paddr_ptr,
			*len_ptr);
		cam_smmu_free_io_iova(idx, *paddr_ptr, *len_ptr);
	}
err_unmap_sg:
	...
	int idx)
{
	...
	if (mapping_info->region_id == CAM_SMMU_REGION_SHARED) {
		...
		/*
		 * 6.18 port: was msm_dma_unmap_sg(). Unmap from our explicit
		 * domain and return the IOVA to the IO-region pool.
		 */
		domain = iommu_cb_set.cb_info[idx].domain;
		...
		if (size != mapping_info->len)
			CAM_ERR(CAM_SMMU,
				"IO IOMMU unmap: unmapped = %zu, requested = %zu",
				size, mapping_info->len);

		cam_smmu_free_io_iova(idx, mapping_info->paddr,
			mapping_info->len);
	}
...
static void cam_smmu_deinit_cb(struct cam_context_bank_info *cb)
{
	...
	if (cb->io_support && cb->io_mem_pool) {
	...
	struct device *dev)
{
	...
	/*
	 * 6.18 port: allocate an explicit paging iommu_domain for this context
	 * bank's device and an IOVA pool over the DT-described IO region.
	 *
	 * Legacy used arm_iommu_create_mapping(bus, io_start, io_len) which both
	 * allocated the domain AND set up the IOVA allocator over [io_start,
	 * io_start+io_len). Mainline splits these: iommu_paging_domain_alloc()
	 * gives the domain; we own the IO IOVA space via cb->io_mem_pool. The
	 * removed iommu_domain_set_attr() tuning hints (non-fatal-fault, iova
	 * guard page) have no generic mainline equivalent and are not required
	 * for correctness, so they are dropped.
	 */
	if (cb->io_support) {
		cb->domain = iommu_paging_domain_alloc(dev);
		if (IS_ERR(cb->domain)) {
			CAM_ERR(CAM_SMMU, "Error: paging domain alloc failed");
		...
		cb->io_mem_pool = gen_pool_create(PAGE_SHIFT, -1);
		if (!cb->io_mem_pool) {
			CAM_ERR(CAM_SMMU, "Error: IO genpool create failed");
			rc = -ENOMEM;
			goto err_domain;
		}

		rc = gen_pool_add(cb->io_mem_pool,
			cb->io_info.iova_start,
			cb->io_info.iova_len,
			-1);
		if (rc) {
			CAM_ERR(CAM_SMMU, "Error: IO genpool chunk add failed");
			gen_pool_destroy(cb->io_mem_pool);
			cb->io_mem_pool = NULL;
			goto err_domain;
		}

		CAM_DBG(CAM_SMMU, "IO mem start->%lX len->%zu",
