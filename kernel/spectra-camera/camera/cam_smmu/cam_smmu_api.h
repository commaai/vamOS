...
/*
 * 6.18 port (Phase 2.B): asm/dma-iommu.h (ARM legacy IOMMU mapping API) and
 * linux/msm_ion.h (downstream ION) were removed in mainline. The ARM mapping
 * API is gone for good; cam_smmu now drives the generic iommu_domain API
 * directly (see cam_smmu_api.c). The ION types still named in this header's
 * public stage-2 contract come from the local compat shim, retired at the
 * 2.A/2.E seam.
 */
#include "cam_smmu_compat.h"
