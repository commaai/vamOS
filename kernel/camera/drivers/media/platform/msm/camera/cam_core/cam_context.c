	...
	uint32_t req_size)
{
	...
	strscpy /* VAMOS: was strlcpy */(ctx->dev_name, dev_name, CAM_CTX_DEV_NAME_MAX_LENGTH);
	ctx->dev_id = dev_id;
	ctx->ctx_id = ctx_id;
...
void cam_context_putref(struct cam_context *ctx)
{
	...
	CAM_DBG(CAM_CORE,
		...
		ctx->dev_hdl, kref_read(&ctx->refcount) /* VAMOS: use kref_read() instead of reaching into kref internals */,
		ctx->dev_name);
}
