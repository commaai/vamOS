...
{
	/* VAMOS: vendor SCM API (scm_call2, SCM_SIP_FNID, etc.) not in mainline.
	 * Secure camera LUT notification is not supported — stub to -ENOSYS. */
	return -ENOSYS;
#if 0 /* original vendor SCM implementation */
	...
	return rc;
#endif /* original vendor SCM implementation */
}
	...
	list_for_each_entry(cid_res_iterator, &ife_ctx->res_list_ife_cid,
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			hw_intf = cid_res_iterator->hw_res[i]->hw_intf;
                    CAM_DBG(CAM_ISP,
                        "ife cid resource from hw %d index %d",
                        hw_intf->hw_idx, i);
			rc = hw_intf->hw_ops.reserve(hw_intf->hw_priv,
    ...
    if (ife_hw_mgr->csid_devices[in_port->custom_csid]) {
        ...
        if (!rc) {
            ...
            cid_res_temp->hw_res[acquired_cnt++] =
                csid_acquire.node_res;
            CAM_DBG(CAM_ISP,
                "acquired csid %d CID rsrc successfully from custom id %u",
                hw_intf->hw_idx, in_port->custom_csid);
        }
    }
	...
	for (i = 0; i < CAM_IFE_CSID_HW_NUM_MAX && (!match); i++) {
		...
		if (rc)
			...
			cid_res_temp->hw_res[acquired_cnt++] =
				csid_acquire.node_res;
                    CAM_DBG(CAM_ISP,
                        "acquired csid %d CID rsrc successfully from acqurie index %d",
                        hw_intf->hw_idx, i);
			break;
		}
	}
...
/* VAMOS: debugfs_create_* returns void in mainline 6.x, remove error checks */
