...
{
	struct timespec64 ts; /* VAMOS: struct timespec removed in mainline */

	ktime_get_boottime_ts64(&ts); /* VAMOS: get_monotonic_boottime() removed in mainline */
	time_stamp->mono_time.tv_sec    = ts.tv_sec;
	time_stamp->mono_time.tv_nsec   = ts.tv_nsec; /* VAMOS: tv_usec -> tv_nsec for timespec64 */
}
