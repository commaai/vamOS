...
/* VAMOS: removed <asm/dma-iommu.h> — ARM DMA-IOMMU removed in mainline 6.x */
#include <linux/dma-direction.h>
#include <linux/of_platform.h>
#include <linux/platform_device.h> /* VAMOS: needed for platform_bus_type */
#include <linux/iommu.h>
#include <linux/slab.h>
#include <linux/dma-mapping.h>
/* VAMOS: removed <linux/msm_dma_iommu_mapping.h> — vendor DMA-IOMMU not in mainline */
#include <linux/workqueue.h>
#include <linux/genalloc.h>
/* VAMOS: removed <soc/qcom/scm.h> — vendor SCM not in mainline */
/* VAMOS: removed <soc/qcom/secure_buffer.h> — vendor secure buffer not in mainline */
...
struct cam_context_bank_info {
	struct device *dev;
	/* VAMOS: was struct dma_iommu_mapping *mapping — replaced with iommu_domain */
	struct iommu_domain *domain;
	dma_addr_t va_start;
	size_t va_len;
	const char *name;
...
/* VAMOS: was ion_handle/ion_client — secure camera stubbed, kept struct for compilation */
...
{
	/* VAMOS: stubbed — arm_iommu_attach_device removed in mainline.
	 * In mainline, IOMMU is attached automatically via DT/IOMMU groups.
	 * TODO: needs ARM DMA-IOMMU -> mainline IOMMU port */
	struct cam_context_bank_info *cb = &iommu_cb_set.cb_info[idx];

	if (!cb->domain) {
		cb->domain = iommu_get_domain_for_dev(cb->dev);
		if (!cb->domain) {
			CAM_ERR(CAM_SMMU, "Error: no IOMMU domain for dev");
			return -ENODEV;
		}
	}
	return 0;
...
static int cam_smmu_detach_device(int idx)
{
	...
	/* VAMOS: stubbed — arm_iommu_detach_device removed in mainline.
	 * IOMMU detach handled automatically by the IOMMU subsystem.
	 * TODO: needs ARM DMA-IOMMU -> mainline IOMMU port */
	...
	size_t *len)
{
	...
	domain = iommu_cb_set.cb_info[idx].domain; /* VAMOS: was mapping->domain */
	rc = iommu_map(domain,
		firmware_start,
		icp_fw.fw_dma_hdl,
		firmware_len,
		IOMMU_READ|IOMMU_WRITE|IOMMU_PRIV,
		GFP_KERNEL); /* VAMOS: 6th arg added for mainline 6.x */
	...
	size_t *request_len)
{
	...
	size = iommu_map_sg(iommu_cb_set.cb_info[idx].domain, /* VAMOS: was mapping->domain */
		sec_heap_iova,
...
int cam_smmu_release_sec_heap(int32_t smmu_hdl)
{
	...
	size = iommu_unmap(iommu_cb_set.cb_info[idx].domain, /* VAMOS: was mapping->domain */
		sec_heap_iova,
		sec_heap_iova_len);
	if (size != sec_heap_iova_len) {
	...
	struct cam_dma_buff_info **mapping_info)
{
	...
	if (region_id == CAM_SMMU_REGION_SHARED) {
		...
		/* VAMOS: replaced msm_dma_map_sg_lazy with dma_map_sg */
		rc = dma_map_sg(iommu_cb_set.cb_info[idx].dev,
		table->sgl, table->nents, dma_dir);

		if (rc != table->nents) {
			CAM_ERR(CAM_SMMU, "Error: dma_map_sg failed");
			rc = -ENOMEM;
			goto err_unmap_sg;
		} else {
...
err_alloc:
	if (region_id == CAM_SMMU_REGION_SHARED) {
		...
		iommu_unmap(iommu_cb_set.cb_info[idx].domain, /* VAMOS: was mapping->domain */
			*paddr_ptr,
			*len_ptr);
	} else if (region_id == CAM_SMMU_REGION_IO) {
		/* VAMOS: replaced msm_dma_unmap_sg with dma_unmap_sg */
	...
	dma_addr_t *virt_addr)
{
	...
	if (iommu_map_sg(domain,
		...
		table->nents,
		iommu_dir,
		GFP_KERNEL) != virt_len) { /* VAMOS: 6th arg added for mainline 6.x */
	...
	int idx)
{
	...
	size_t unmapped;
	struct iommu_domain *domain =
		iommu_cb_set.cb_info[idx].domain; /* VAMOS: was mapping->domain */
	struct scratch_mapping *scratch_map =
...
static int cam_smmu_map_stage2_buffer_and_add_to_list(int idx, int ion_fd,
		 enum dma_data_direction dma_dir, void *client,
		 dma_addr_t *paddr_ptr,
		 size_t *len_ptr)
{
	/* VAMOS: stubbed — secure camera (stage2/ION) not supported in mainline */
	return -ENOSYS;
}

/* VAMOS: stubbed — secure camera (stage2) not supported, ION removed */
int cam_smmu_map_stage2_iova(int handle,
		int ion_fd, enum cam_smmu_map_dir dir,
		void *client, phys_addr_t *paddr_ptr,
		size_t *len_ptr)
{
	return -ENOSYS;
}
...
static int cam_smmu_secure_unmap_buf_and_remove_from_list(
		...
		int idx)
{
	/* VAMOS: stubbed — secure camera (ION) not supported in mainline */
	if (!mapping_info) {
...
{
	/* VAMOS: arm_iommu_detach_device/arm_iommu_release_mapping removed in mainline.
	 * IOMMU lifecycle managed by the subsystem automatically. */
	cb->domain = NULL;
	...
	struct device *dev)
{
	...
	/* VAMOS: was arm_iommu_create_mapping + iommu_domain_set_attr —
	 * both removed in mainline 6.x. Domain is obtained at attach time
	 * via iommu_get_domain_for_dev(). */
	if (cb->io_support) {
		cb->domain = iommu_get_domain_for_dev(cb->dev);
		if (!cb->domain) {
			CAM_DBG(CAM_SMMU,
				"No IOMMU domain yet for %s, will retry at attach",
				cb->name);
		}
	} else {
...
static void cam_smmu_remove(struct platform_device *pdev) /* VAMOS: was int, void in mainline 6.x */
{
