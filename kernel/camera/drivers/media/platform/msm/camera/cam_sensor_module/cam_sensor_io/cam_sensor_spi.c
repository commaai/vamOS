	...
	uint32_t num_byte, char *tx, char *rx)
{
	...
	/* VAMOS: replaced cma_alloc/cma_release with dma_alloc_coherent */
	...
	if (tx) {
		...
		ctx = dma_alloc_coherent(dev, len, &tx_dma, GFP_KERNEL);
		if (!ctx)
			return -ENOMEM;
	}
	...
	if (num_byte) {
		if (rx) {
			...
			crx = dma_alloc_coherent(dev, len, &rx_dma, GFP_KERNEL);
			if (!crx) {
				if (!tx)
					dma_free_coherent(dev, len,
						ctx, tx_dma);
...
out:
	if (!tx)
		dma_free_coherent(dev, len, ctx, tx_dma);
	if (!rx)
		dma_free_coherent(dev, len, crx, rx_dma);
	return rc;
}
