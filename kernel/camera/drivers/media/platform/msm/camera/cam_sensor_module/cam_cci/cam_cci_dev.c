	...
	bool *handled)
{
	...
	ret = cam_cci_irq(soc_info->irq_num /* VAMOS: was irq_line->start */, cci_dev);
	*handled = TRUE;
	return 0;
}
...
static int cam_cci_platform_probe(struct platform_device *pdev)
{
	...
	new_cci_dev->v4l2_dev_str.ops =
		&cci_subdev_ops;
	strscpy /* VAMOS: was strlcpy */(new_cci_dev->device_name, CAMX_CCI_DEV_NAME,
	...
	strscpy /* VAMOS: was strlcpy */(cpas_parms.identifier, "cci", CAM_HW_IDENTIFIER_LENGTH);
...
static void cam_cci_device_remove(struct platform_device *pdev) /* VAMOS: was int, void in mainline 6.x */
{
