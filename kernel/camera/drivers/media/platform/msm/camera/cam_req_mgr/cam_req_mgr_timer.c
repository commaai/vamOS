...
/* VAMOS: setup_timer replaced by timer_setup in mainline 4.15+ */
void crm_timer_callback(struct timer_list *t)
{
	/* VAMOS: from_timer() removed in mainline 6.x, use container_of directly */
	struct cam_req_mgr_timer *timer = container_of(t, struct cam_req_mgr_timer, sys_timer);
	...
	int32_t expires, void *parent, void (*timer_cb)(struct timer_list *))
{
	...
	if (*timer == NULL) {
		...
		/* VAMOS: was setup_timer, timer_setup in mainline 4.15+ */
...
void crm_timer_exit(struct cam_req_mgr_timer **crm_timer)
{
	...
	if (*crm_timer) {
		timer_delete_sync /* VAMOS: was del_timer_sync */(&(*crm_timer)->sys_timer);
		if (g_cam_req_mgr_timer_cachep)
