...
/* VAMOS: removed <linux/msm_ion.h> — vendor ION extensions not in mainline */
#include <linux/slab.h>
#include <linux/dma-buf.h>
#include <linux/dma-mapping.h> /* VAMOS: for dma_alloc_coherent */
...
/* VAMOS: replaced ion_map_kernel/ion_handle_get_size with dma_buf_vmap */
	...
	size_t *len)
{
	...
	if (!buf) {
		CAM_ERR(CAM_MEM, "Invalid dma_buf");
		return -EINVAL;
	}

	rc = dma_buf_vmap(buf, &map);
	if (rc) {
...
/* VAMOS: helper to unmap kernel va */
static void cam_mem_util_unmap_cpu_va(struct dma_buf *buf, uint64_t kmdvaddr)
{
	struct iosys_map map = IOSYS_MAP_INIT_VADDR((void *)(uintptr_t)kmdvaddr);
...
/* VAMOS: replaced msm_ion_client_create — no client needed for dma_buf/dma_alloc.
 * We just need a device for DMA allocation, which will be set later when
 * the camera subsystem probes. For now, this is a no-op. */
...
/* VAMOS: replaced ion_client_destroy — no-op */
...
int cam_mem_mgr_init(void)
{
	...
	if (rc < 0) {
		CAM_ERR(CAM_MEM, "fail to create dma heap client");
		goto client_fail;
	}
...
int cam_mem_get_cpu_buf(int32_t buf_handle, uint64_t *vaddr_ptr, size_t *len)
{
	...
	/* VAMOS: replaced ion_handle with dma_buf for kernel mapping */
	if (tbl.bufq[idx].flags & CAM_MEM_FLAG_KMD_ACCESS) {
		if (!tbl.bufq[idx].kmdvaddr) {
			...
			rc = cam_mem_util_map_cpu_va(tbl.bufq[idx].dma_buf,
				&kvaddr, &klen);
			if (rc)
				goto exit_func;
...
/* VAMOS: replaced ION cache ops with dma_buf_begin/end_cpu_access */
int cam_mem_mgr_cache_ops(struct cam_mem_cache_ops_cmd *cmd)
{
	...
	/* VAMOS: use dma_buf_begin/end_cpu_access for cache maintenance */
	dir = DMA_BIDIRECTIONAL;
	switch (cmd->mem_cache_ops) {
	case CAM_MEM_CLEAN_CACHE:
		rc = dma_buf_end_cpu_access(tbl.bufq[idx].dma_buf, dir);
		break;
	case CAM_MEM_INV_CACHE:
		rc = dma_buf_begin_cpu_access(tbl.bufq[idx].dma_buf, dir);
		break;
...
/* VAMOS: stubbed — ION allocator removed. In mainline, userspace allocates via
 * /dev/dma_heap/system and passes the fd to the kernel. Kernel-internal allocations
 * should use dma_alloc_coherent or alloc_pages + dma_buf_export.
 * TODO: implement proper kernel-side dma_buf allocation if needed. */
static int cam_mem_util_get_dma_buf(size_t len,
	unsigned int flags,
	struct dma_buf **buf)
{
	/* VAMOS: stubbed — needs proper dma_buf allocation implementation */
	CAM_ERR(CAM_MEM, "kernel-side dma_buf alloc not yet implemented");
	return -ENOSYS;
}
	...
	int *fd)
{
	...
	rc = cam_mem_util_get_dma_buf(len, flags, buf);
	if (rc)
		return rc;
...
/* VAMOS: replaced ION heap/flag logic — allocation stubbed for now */
	...
	int *fd)
{
	...
	if (cmd->flags & CAM_MEM_FLAG_PROTECTED_MODE) {
		/* VAMOS: secure allocation not supported in mainline */
		CAM_ERR(CAM_MEM, "Secure allocation not supported");
		return -ENOSYS;
	}

	return cam_mem_util_get_dma_buf_fd(cmd->len, flags, buf, fd);
}
	...
	enum cam_smmu_region_id region)
{
	...
	if (flags & CAM_MEM_FLAG_PROTECTED_MODE) {
		/* VAMOS: secure camera (stage2) not supported in mainline */
		CAM_ERR(CAM_MEM, "Protected mode not supported");
		return -ENOSYS;
	} else {
...
int cam_mem_mgr_alloc_and_map(struct cam_mem_mgr_alloc_cmd *cmd)
{
	...
	/* VAMOS: replaced ion_alloc with dma-heap alloc */
	rc = cam_mem_util_ion_alloc(cmd, &buf, &ion_fd);
	if (rc) {
		CAM_ERR(CAM_MEM, "DMA heap allocation failed");
		return rc;
	}
	...
	tbl.bufq[idx].dma_buf = buf; /* VAMOS: store dma_buf directly */
...
slot_fail:
	dma_buf_put(buf); /* VAMOS: replaced ion_free with dma_buf_put */
	return rc;
}
...
int cam_mem_mgr_map(struct cam_mem_mgr_map_cmd *cmd)
{
	...
	/* VAMOS: replaced ion_import_dma_buf_fd with dma_buf_get */
	buf = dma_buf_get(cmd->fd);
	if (IS_ERR_OR_NULL(buf)) {
		CAM_ERR(CAM_MEM, "Failed to get dma_buf for fd");
		return -EINVAL;
	}
	...
	if ((cmd->flags & CAM_MEM_FLAG_HW_READ_WRITE) ||
		...
		len = buf->size; /* VAMOS: replaced ion_handle_get_size */
	}
	...
	tbl.bufq[idx].dma_buf = buf; /* VAMOS: store dma_buf */
	tbl.bufq[idx].flags = cmd->flags;
...
map_fail:
	dma_buf_put(buf); /* VAMOS: replaced ion_free */
	return rc;
}
...
static int cam_mem_mgr_cleanup_table(void)
{
	...
	for (i = 1; i < CAM_MEM_BUFQ_MAX; i++) {
		...
		/* VAMOS: replaced ion_free with dma_buf_put */
	...
	enum cam_smmu_mapping_client client)
{
	...
	/* VAMOS: replaced ion_unmap_kernel with dma_buf_vunmap */
	if (tbl.bufq[idx].flags & CAM_MEM_FLAG_KMD_ACCESS)
	...
	CAM_DBG(CAM_MEM,
		"idx = %d freeing, fd = %d, imported %d dma_buf %pK",
		idx, tbl.bufq[idx].fd,
...
/* VAMOS: replaced ION alloc + ion_map_kernel with dma-heap + dma_buf_vmap */
	...
	struct cam_mem_mgr_memory_desc *out)
{
	...
	/* VAMOS: TODO — kernel-side dma_buf allocation stubbed for now.
	 * In openpilot, userspace handles allocation via /dev/dma_heap/system.
	 * This code path may not be used in practice. */
	rc = cam_mem_util_get_dma_buf(inp->size, 0, &buf);

	if (rc) {
		CAM_ERR(CAM_MEM, "DMA alloc failed for shared buffer");
		goto ion_fail;
	} else {
...
/* VAMOS: replaced ION alloc with dma-heap alloc for sec heap reservation */
	...
	struct cam_mem_mgr_memory_desc *out)
{
	...
	if (rc) {
		CAM_ERR(CAM_MEM, "DMA alloc failed for sec heap buffer");
		goto ion_fail;
	} else {
		CAM_DBG(CAM_MEM, "Got dma_buf = %pK", buf);
	}
