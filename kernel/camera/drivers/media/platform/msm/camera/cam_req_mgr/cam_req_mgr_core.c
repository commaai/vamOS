 ...
 *
 */
/* VAMOS: callback signature changed for timer_setup (mainline 4.15+) */
static void __cam_req_mgr_sof_freeze(struct timer_list *t)
{
	/* VAMOS: split decl from from_timer to avoid type-inference issues */
	struct cam_req_mgr_timer     *timer =
		container_of(t, struct cam_req_mgr_timer, sys_timer);
