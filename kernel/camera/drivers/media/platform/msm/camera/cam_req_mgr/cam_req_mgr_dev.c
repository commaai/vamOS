...
#define CAMERA_ENABLE_PC_LATENCY PM_QOS_DEFAULT_VALUE /* VAMOS: pm_qos renamed to cpu_latency_qos in mainline 5.x */
...
static int cam_media_device_setup(struct device *dev)
{
	...
	strscpy /* VAMOS: was strlcpy */(g_dev.v4l2_dev->mdev->model, CAM_REQ_MGR_VNODE_NAME,
...
/* VAMOS: pm_qos renamed to cpu_latency_qos in mainline 5.x */
static void cam_pm_qos_add_request(void)
{
	cpu_latency_qos_add_request(&cam_pm_qos_request,
...
static int cam_video_device_setup(void)
{
	...
	strscpy /* VAMOS: was strlcpy */(g_dev.video->name, "cam-req-mgr",
	...
	g_dev.video->device_caps = V4L2_CAP_VIDEO_CAPTURE; /* VAMOS: required in mainline 6.x */
	rc = video_register_device(g_dev.video, VFL_TYPE_VIDEO /* VAMOS: VFL_TYPE_GRABBER renamed to VFL_TYPE_VIDEO in mainline 6.x */, -1);
	if (rc)
		goto v4l2_fail;
...
static void cam_req_mgr_remove(struct platform_device *pdev) /* VAMOS: was int, void in mainline 6.x */
{
	...
	if (g_cam_req_mgr_timer_cachep == NULL) {
		...
		if (!g_cam_req_mgr_timer_cachep)
			...
			CAM_DBG(CAM_CRM, "Name : %s",
				"crm_timer" /* VAMOS: kmem_cache_name not in mainline, use literal */); /* VAMOS: struct kmem_cache is opaque in mainline, use accessor */
	}
...
MODULE_DEVICE_TABLE(of, cam_req_mgr_dt_match); /* VAMOS: was cam_dt_match, use actual table name */
