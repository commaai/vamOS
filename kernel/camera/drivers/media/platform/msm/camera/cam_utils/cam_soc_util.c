...
#include <linux/pinctrl/consumer.h> /* VAMOS: needed for devm_pinctrl_get, pinctrl_lookup/select_state */
/* VAMOS: removed <soc/qcom/socinfo.h> — vendor socinfo not in mainline */
...
{
	return 321; /* VAMOS: socinfo_get_id() not in mainline, hardcoded SDM845 */
}
...
int cam_soc_util_irq_enable(struct cam_hw_soc_info *soc_info)
{
	...
	enable_irq(soc_info->irq_num /* VAMOS: was irq_line->start */);
...
int cam_soc_util_irq_disable(struct cam_hw_soc_info *soc_info)
{
	...
	disable_irq(soc_info->irq_num /* VAMOS: was irq_line->start */);
	...
	uint32_t clk_index, unsigned long flags)
{
	...
	/* VAMOS: clk_set_flags() is a vendor API not available in mainline, stub out */
	return 0;
}
	...
	struct cam_hw_soc_info *soc_info, bool gpio_en)
{
	...
	if (gpio_en) {
		...
		/* VAMOS: gpio_free_array removed from mainline, free individually */
		int j;
		for (j = 0; j < size; j++)
...
int cam_soc_util_get_dt_properties(struct cam_hw_soc_info *soc_info)
{
	...
	for (i = 0; i < soc_info->num_mem_block; i++) {
		...
		if (!soc_info->mem_block[i]) {
			CAM_ERR(CAM_UTIL, "no mem resource by name %s (pdev=%s, num_resources=%d)",
				soc_info->mem_block_name[i],
				dev_name(&soc_info->pdev->dev),
				soc_info->pdev->num_resources);
			rc = -ENODEV;
			return rc;
		}
	}
	...
	if (rc) {
		...
		/* VAMOS: mainline 6.x no longer populates IRQs as platform resources.
		 * Use platform_get_irq_byname() instead of platform_get_resource_byname(IORESOURCE_IRQ). */
		int irq = platform_get_irq_byname(soc_info->pdev, soc_info->irq_name);
		if (irq < 0) {
			CAM_ERR(CAM_UTIL, "no irq for %s (pdev=%s, rc=%d)",
				soc_info->irq_name,
				dev_name(&soc_info->pdev->dev), irq);
			return irq;
		}
		soc_info->irq_num = irq;
		/* Keep irq_line non-NULL as a flag that IRQ exists */
		soc_info->irq_line = (struct resource *)1;
	}
	...
	irq_handler_t handler, void *irq_data)
{
	...
	if (soc_info->irq_line) {
		rc = devm_request_irq(soc_info->dev, soc_info->irq_num /* VAMOS: was irq_line->start */,
...
put_clk:
	...
	if (soc_info->irq_line) {
		...
		devm_free_irq(soc_info->dev,
			soc_info->irq_num /* VAMOS: was irq_line->start */, irq_data);
	}
...
int cam_soc_util_release_platform_resource(struct cam_hw_soc_info *soc_info)
{
	...
	if (soc_info->irq_line) {
		disable_irq(soc_info->irq_num /* VAMOS: was irq_line->start */);
		devm_free_irq(soc_info->dev,
			soc_info->irq_num /* VAMOS: was irq_line->start */, soc_info->irq_data);
	}
