...
/* VAMOS: removed <linux/clk/qcom.h> — vendor clock helpers not in mainline */
#include <linux/debugfs.h>

#include "cam_io_util.h"

/* VAMOS: Legacy GPIO compat — struct gpio, of_gpio_count, of_get_gpio removed
 * from mainline 6.x. Define locally for compat with Spectra driver. */
#include <linux/gpio/consumer.h>
#include <linux/of.h>

#ifndef __CAM_COMPAT_LEGACY_GPIO
#define __CAM_COMPAT_LEGACY_GPIO
...
{
	return of_property_count_u32_elems(np, "gpios") / 2;
}
...
static inline int of_get_gpio(const struct device_node *np, int index)
{
	int ret;
	u32 gpio;
	ret = of_property_read_u32_index(np, "gpios", index * 2, &gpio);
	return ret ? ret : gpio;
}
#endif
...
struct cam_hw_soc_info {
	...
	int                            irq_num; /* VAMOS: mainline 6.x no longer populates IRQ as platform resource */
	void                           *irq_data;
