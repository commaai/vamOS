	...
	struct platform_device *pdev)
{
	...
	strscpy /* VAMOS: was strlcpy */(sync_dev->v4l2_dev.mdev->model, CAM_SYNC_DEVICE_NAME,
...
static int cam_sync_create_debugfs(void)
{
	...
	/* VAMOS: debugfs_create_bool() returns void in mainline 6.x, no error check needed */
...
static int cam_sync_probe(struct platform_device *pdev)
{
	...
	strscpy /* VAMOS: was strlcpy */(sync_dev->vdev->name, CAM_SYNC_NAME,
	...
	sync_dev->vdev->device_caps = V4L2_CAP_VIDEO_CAPTURE; /* VAMOS: required in mainline 6.x */
	rc = video_register_device(sync_dev->vdev,
		VFL_TYPE_VIDEO /* VAMOS: VFL_TYPE_GRABBER renamed to VFL_TYPE_VIDEO in mainline 6.x */, -1);
	if (rc < 0)
		goto v4l2_fail;
...
static void cam_sync_remove(struct platform_device *pdev) /* VAMOS: was int, void in mainline 6.x */
{
	v4l2_device_unregister(sync_dev->vdev->v4l2_dev);
	cam_sync_media_controller_cleanup(sync_dev);
	video_device_release(sync_dev->vdev);
