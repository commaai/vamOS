...
#include <linux/of.h>
/* VAMOS: removed <linux/msm-bus.h> — vendor bus voting not in mainline */
...
/* VAMOS: All bus voting functions stubbed — msm_bus_scale API not in mainline.
 * Bandwidth voting is a performance optimization; camera functions without it.
 * TODO: replace with mainline interconnect (icc) framework for production use. */
...
/* VAMOS: AXI port setup stubbed — msm_bus bandwidth voting not in mainline.
 * The qcom,axi-port-list DT node and bus client registration are skipped. */
	...
	struct cam_hw_soc_info *soc_info)
{
	...
	if (!axi_port_list_node) {
		CAM_DBG(CAM_CPAS, "Node qcom,axi-port-list not found, skipping (bus voting stubbed)");
		return 0;
	}
	...
	struct cam_hw_intf **hw_intf)
{
	...
	cpas_core->work_queue = alloc_workqueue("cam-cpas",
		WQ_SYSFS | WQ_MEM_RECLAIM | WQ_SYSFS | WQ_HIGHPRI, CAM_CPAS_INFLIGHT_WORKS);
